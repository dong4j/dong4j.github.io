...
ai:
  - 声网客户端API指南包括账号消息系统、频道管理和呼叫系统。登录时需提供账号名、token和uid等信息；频道加入与离开操作分别用于用户在多通道间的切换，并可调整频道属性或查询成员数量；呼叫功能允许发起、接受、拒绝或结束通话邀请，且涉及多种回调处理失败场景。
description: 声网客户端API指南包括账号消息系统、频道管理和呼叫系统。登录时需提供账号名、token和uid等信息；频道加入与离开操作分别用于用户在多通道间的切换，并可调整频道属性或查询成员数量；呼叫功能允许发起、接受、拒绝或结束通话邀请，且涉及多种回调处理失败场景。
---
