...
tags:
  ...
  - 异常处理改进
  - 性能分析工具
description: 制定一个日志规范, 将日志相关的常识性问题做一个总结.
abbrlink: 286572f8
date: 2018-10-21 00:00:00
ai:
  - 这篇文章深入探讨了日志在软件开发和系统调试中的重要性，并提出了严格的日志使用规范。它强调了记录异常、输出所有Throwable信息、避免重复记录日志以及使用SLF4J替代log4j的重要性。同时，文章指出仅仅依赖单个应用的日志无法解决跨进程或跨线程的场景问题，因此提出将考虑构建一个日志追踪系统来汇总并分析海量日志数据，以优化性能和排查问题。
---
