window.IP_CONFIG = {
  API_KEY: "YVBk35HahUF3wsesbhH6Ih9Luh", // API密钥 申请地址：https://api.76.al/
  BLOG_LOCATION: {
    lng: 104.068957, // 经度
    lat: 30.537736, // 纬度
  },
...
const fetchIpData = async () => {
  const response = await fetch(
    `https://api.nsmao.net/api/ip/query?key=${encodeURIComponent(
      IP_CONFIG.API_KEY
    )}`
  );
...
const showWelcome = ({ data, ip }) => {
  ...
  welcomeInfo.innerHTML = generateWelcomeMessage(
    pos,
    dist,
    ipDisplay,
    country,
...
const showErrorMessage = (message = "抱歉，无法获取信息") => {
  ...
  document
    ...
    .addEventListener("click", fetchIpInfo);
};
